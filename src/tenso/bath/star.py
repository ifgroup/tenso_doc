...
class Tedopa(DiscretizationSolver):
    ...
    def _tridiagonalize(self) -> tuple[NDArray, NDArray]:
        ...
        """
        a = self._get_chain_frequency()
        b = self._get_chain_coupling()[1:]
...
if __name__ == '__main__':
    from tenso.bath.sd import OhmicExp, Drude, UnderdampedBrownian
    import matplotlib.pyplot as plt
    ...
    # spd = OhmicExp(1, 0.1)
    spd = UnderdampedBrownian(1, 0.33, 0.1)
    # spd = Drude(1, 0.1)
    ...
    plt.plot(t_fs,
             c_space.real,
             'r-.',
             label=f'Re. (TEDOPA (n={n_discr}))',
             lw=1.5)
    plt.plot(t_fs,
             c_space.imag,
             'b-.',
             label=f'Im. (TEDOPA (n={n_discr}))',
             lw=1.5)
