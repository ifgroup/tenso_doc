...
if __name__ == "__main__":
    ...
    from tenso.bath.sd import Drude
    ...
    sd = Drude(quantity(200, 'energy'),
                quantity(100, 'energy'))
    beta = quantity(1 / 300.0, 'inverse_temperature')
    sd.FREQ_MAX=10
    t_space_fs = np.linspace(0, 4000, 500)
    t_space = t_space_fs * quantity(1, 'time')

    fig, axes = plt.subplots(2, 1, figsize=(4, 4), sharex=True)
    
    c = [sd.autocorrelation(ti, beta) for ti in t_space]
    c = np.array(c)
    re_c = c.real
    im_c = c.imag
    c0 = re_c[0]
    axes[1].plot(t_space_fs, im_c / c0, label='Exact', color='black')
    axes[0].plot(t_space_fs, re_c / c0, label='Exact', color='black')


    for n_dis in [60, 120, 240]:
        bath = gen_star_boson(
            re_d=[200],
            width_d=[100],
            temperature=300,
            cutoff=1000,
            n_discretization=n_dis,
            discretization_method='TEDOPA',
            shift_frequency=True,
        ) 
        c_real = bath.real_correlation_function(t_space)
        c_imag = bath.imag_correlation_function(t_space)
        axes[0].plot(t_space_fs, c_real / c0, '--', label=f'BSDO ({bath.k_max})')
        axes[1].plot(t_space_fs, c_imag / c0, '--', label=f'BSDO ({bath.k_max})')
    axes[0].set_ylabel(r'Re $C(t)/C(0)$')
    axes[1].set_ylabel(r'Im $C(t)/C(0)$')
    axes[1].set_xlabel('Time (fs)')
    axes[0].set_xlim(0, 4000)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(loc='lower center', ncol=2, bbox_to_anchor=(0.5, -0.1),
               frameon=False,
                handles=handles, labels=labels)
    plt.tight_layout()
    plt.savefig('drude_bath_bsdo.pdf',  bbox_inches='tight')
    plt.show()