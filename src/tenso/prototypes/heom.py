...
def spin_boson(
    ...
    **kwargs,
    ...
    if td_f is not None and td_op is not None:
        # td_f in `default_parameters` and td_op is unitless
        _op = opt_array(td_op)
...
def system_multibath(
    ...
    **kwargs,
    ...
    for _t, _s in prop_it:
        ...
        output_logger.info(f'{time} ' + " ".join(f'{p_i:.8f}' for p_i in flat))
        debug_logger.info(
            f'{time} {propagator.ode_step_counter} {max(ranks) if ranks else 0} {trace} {norm} {_s.size()}'
        )
        if ranks:
            debug_logger.info('# ' + ' '.join([f'{_r:d}' for _r in ranks]))
    ...
    **kwargs,
    ...
    if td_fields and td_ops:
        ...
        def f_list(time: float):
            fields = [_f(time/ut) * ue for _f in td_fields]
            ans = []
